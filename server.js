...
const HISTORY_FILE = path.join(ROOT,'public','prod-rnd-backend-php-orchestra.100hp.app','mines','sessions.html');
...
function safeResolve(urlPath) {
  ...
  if (decoded.startsWith('/')) {
    target = path.join(ROOT, decoded.substring(1));
  } else {
...
const Store = {
  ...
  settings: (() => {
    try {
      const p = path.join(ROOT, 'public', 'prod-rnd-backend-php-orchestra.100hp.app', 'mines', 'settings.html');
...
async function requestHandler(req,res){
  try{
    ...
    if (urlPath === '/mines/mock-sw.js') {
      const p = path.join(ROOT,'public','mines','mock-sw.js');
    ...
    let pth = urlPath;
    if (pth.startsWith('/static/')) pth = path.posix.join('/public', pth);
    if (pth === '/favicon.svg') pth = path.posix.join('/public', pth);
    
    // Handle /mines/ path specifically - redirect to game
    if (pth === '/mines/' || pth === '/mines') {
      pth = '/public/mines/index.html';
    }
    ...
    console.log('[DEBUG] Trying to serve:', { urlPath, pth, resolved, filePath, exists: fs.existsSync(filePath) });
    
    if (!fs.existsSync(filePath)) {
      // Try to find mines game in different locations
      const minesPaths = [
        path.join(ROOT,'public','mines','index.html'),
      ...
      console.log('[DEBUG] File not found, checking mines paths:', minesPaths.map(p => ({ path: p, exists: fs.existsSync(p) })));
      
      for (const minesPath of minesPaths) {
        if (fs.existsSync(minesPath)) {
          console.log('[DEBUG] Found mines game at:', minesPath);
          return send(res,200,fs.readFileSync(minesPath),{ 'Content-Type':'text/html; charset=utf-8', 'Cache-Control':'no-store' });
        }
      }
      
      // If no mines game found, serve a basic HTML page
      const basicHtml = `<!DOCTYPE html>
<html lang="ru">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>CAVEMINES - Игра не найдена</title>
    <style>
        ...
        .info { color: #ccc; font-size: 16px; }
    </style>
</head>
<body>
    <div class="error">⚠️ Игра CAVEMINES не найдена</div>
    <div class="info">Проверьте файлы в папке prod-rnd-frontend-php-orchestra.100hp.app/mines/</div>
    <div class="info">URL: ${urlPath}</div>
    <div class="info">Ищем в: ${minesPaths.join(', ')}</div>
    <div class="info">ROOT: ${ROOT}</div>
</body>
</html>`;
      return send(res,200,basicHtml,{ 'Content-Type':'text/html; charset=utf-8', 'Cache-Control':'no-store' });
    }
    
    if (fs.statSync(filePath).isDirectory()) {
      return send(res,404,'Directory not found');
    }
    ...
    if (/html/.test(ct) && (/public[\\\/]mines[\\\/]index\.html$/.test(filePath) || /prod-rnd-frontend-php-orchestra\.100hp\.app[\\\/]mines[\\\/]index\.html$/.test(filePath))) {
    ...
    console.error('[server] 500', e && e.message, e && e.stack);
    if (!res.headersSent) return send(res,500,'Internal Server Error');
    try { res.end(); } catch {}
    return;
  }
